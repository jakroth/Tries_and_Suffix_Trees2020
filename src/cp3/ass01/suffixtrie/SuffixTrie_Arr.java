...
public class SuffixTrie_Arr {

    private SuffixTrieNode_Arr root = new SuffixTrieNode_Arr();
     ...
     */
    public SuffixTrieNode_Arr insert(String str, int startPosition) {
        char[] sentence = str.toCharArray();
        SuffixTrieNode_Arr itNode = root;                                           // instantiate iterative node
        for(int i = 0; i < sentence.length; i++) {                              // iterate over every substring in the sentence
            ...
            for (int j = i; j < sentence.length; j++) {                         // iterate over every character in each substring
                if (itNode.getChild(sentence[j]) == null) {                     // see if the character already exists
                    itNode.addChild(sentence[j], new SuffixTrieNode_Arr());         // add new node if not
                }
     ...
     */
    public SuffixTrieNode_Arr get(String str) {
        char[] sentence = str.toCharArray();
        SuffixTrieNode_Arr itNode = root;                       // instantiate iterative node
     ...
     */
    public static SuffixTrie_Arr readInFromFile(String fileName) {
        SuffixTrie_Arr st = new SuffixTrie_Arr();
        Scanner fileScanner;
        int count = 0;
        try {
